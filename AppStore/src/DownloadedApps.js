      ...
      return (
        <div>
          <div style={{marginTop:"10px",marginLeft:"15px"}}>
            <button id="defaultCategory-downloaded" className="categoryButtons" key="All-Downloaded" data-category="All" onClick={this.changeCategoryView} style={{marginRight:"15px"}}>All</button>
            {
              this.props.categories.map((category,index)=>(
              <button className="categoryButtons" key={category+"-Downloaded"} data-category={category} onClick={this.changeCategoryView} style={{display:"inline-block",marginRight:"15px"}}>{category}</button>
              ))
            }
                ...
                this.props.downloadedApps.filter( 
                  ...
                  <Card style={{height:"65px", width:"300px",padding:"10px", border: "none", boxShadow: "none" }}className="appCard" onClick={()=>{this.props.openAppPage(app.id)}}>
                    <div style={{display:"inline-block",verticalAlign:"top"}}>
                      <img src={app.appLogoHash=='default'?defaultLogo:`https://ipfs.fleek.co/ipfs/${app.appLogoHash}`} width={60}height={62}/>
                    </div>
